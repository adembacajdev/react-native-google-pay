...
public class GooglePayModule extends ReactContextBaseJavaModule {
  ...
  private Promise callbackPromise = null;
        ...
        case LOAD_PAYMENT_DATA_REQUEST_CODE:
          switch (resultCode) {
            ...
            case Activity.RESULT_CANCELED:
              callbackPromise.reject("PAYMENT_RESULT_CANCELED", "Payment has been canceled");
              break;
            case AutoResolveHelper.RESULT_ERROR:
              ...
              callbackPromise.reject("PAYMENT_RESULT_ERROR", errorMessage);
              break;
            default:
              Log.w(TAG, "[GooglePay] " + "There is no result code, or it was not handled properly.");
              break;
          }
          break;
      }
  ...
  private void handlePaymentSuccess(PaymentData paymentData) {
    ...
    if (paymentInfo == null) {
      callbackPromise.reject("ERROR => paymentInfo", "paymentInfo is null");
      return;
    }
    JSONObject paymentMethodData;

    try {
      paymentMethodData = new JSONObject(paymentInfo).getJSONObject("paymentMethodData");
